...
//Variable to hold all current polls
var polls = [];

//Variable to hold any polls that match search terms
var pollsWithSearch = [];
...
function updateUserColor() {
    ...
    function addSearchBar() {
        ...
        document.getElementById("search").onkeyup = function (e) {
            clearTimeout(timer);
            timer = setTimeout(updatesearchPoll, 500);
        }
    }
    ...
    socket.on("login", (data) =>{
        ...
        //Re-color particles to user's preferred color
        for (var i = 0; i < particles.length; i++) {
            particles[i].color = options.userColor;
        }
